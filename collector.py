...
""",
    ...
    for i, result in enumerate(results):
        ...
        if isinstance(result, Exception):
            ...
            processed_count, recommended_count = result
            print(f"任务 '{task_name}' 正常结束，本次运行共处理了 {processed_count} 个新商品，其中 {recommended_count} 个被AI推荐。")
            
            # 发送任务完成通知
            try:
                from src.notifier import notifier
                await notifier.send_task_completion_notification(task_name, processed_count, recommended_count)
            except ImportError as e:
                print(f"导入通知模块失败: {e}")
            except Exception as e:
                print(f"发送任务 '{task_name}' 完成通知失败: {e}")
