...
def _load_virtual_text_content(raw_path: str, owner_id: str = None) -> str:

    """读取虚拟路径文本，PostgreSQL 模式对 prompts/* 走数据库优先。"""

    normalized = str(raw_path or "").strip().replace("\\", "/")

    if not normalized:

        raise FileNotFoundError("空路径")

    if STORAGE_BACKEND() == "postgres" and normalized.startswith("prompts/"):

        try:

            from src.storage import get_storage

            filename = normalized.split("/", 1)[-1].split("/")[-1]

            storage = get_storage()

            template = storage.get_prompt_template(filename, owner_id=owner_id)
            ...
            logger.warning(
                ...
                "从数据库读取 Prompt 模板失败，回退文件路径读取",

                extra={"event": "collector_prompt_db_read_failed", "path": normalized, "owner_id": owner_id},

                exc_info=exc

            )

    file_path = resolve_virtual_task_file(normalized, owner_id=owner_id, for_write=False)
    ...
    for task in tasks_config:
        ...
        if task.get("enabled", False) and task.get("ai_prompt_criteria_file"):
            ...
            try:
                ...
                criteria_text = _load_virtual_text_content(
                ...
                if is_requirement_legacy:
                    ...
                    if base_file:
                        base_prompt = _load_virtual_text_content(
                            base_file,
                            owner_id=owner_id
                        )
            ...
            try:
                ...
                task['ai_prompt_text'] = _load_virtual_text_content(
                    task["ai_prompt_file"],
                    owner_id=owner_id
                )
