...
- 精力有限我对项目测试不一定完全，有问题欢迎提issue，感谢
- 二次开发与引用请带上署名，感谢
...
<b>本次更新`0.9.9`ai模型的分析能力增强某些模型默认输出上限较低，必须要拓展输出token字段,大部分模型都支持比默认更高的输出字段，推荐10000起，设置详见[上限设置与判断](#如何判断你的ai-API默认输出token上限是否足够)</b>

<details open>
<summary>v0.9.9 更新日志 - 2026-01-31</summary>

  <ul>
    <li><strong>添加了解锁API输出token上限功能</strong>：本次更新后AI分析能力增强，需拓展输出token字段，大部分模型支持比默认更高的输出字段设置，详情见<a href="#如何判断你的ai-api默认输出token上限是否足够">上限设置与判断</a></li>
